...
/** Setter */
   /** set 모드 */
   function setMode(mode) {AppState.mode = mode; AppState.pagesPath = `docs/${modeNumber(mode)}/pages.json`;}
   /** set 페이지 */
   function setPages(pages) {AppState.pages = pages;}
   /** set 현재 페이지 주소 */
   function setCurrentSlug(slug) {AppState.currentSlug = slug;}
   /** set 로딩 */
   function setLoading(isLoading) {AppState.isLoading = isLoading; isLoading ? showLoading() : hideLoading();}
   /** set 페이지 경로 */
   function setPagesPath(path) {AppState.pagesPath = path;}

const DOM = {pages: document.getElementById('pages'), article: document.getElementById('article'), title: document.getElementById('title'), subtitle: document.getElementById('subtitle'), search: document.getElementById('q'), loading: document.getElementById('page-loading'), homeBtn: document.getElementById('home-btn'), logo: document.querySelector('.logo')};
DOM.pages.innerHTML = '';
DOM.article.innerHTML = '';
DOM.title.textContent = '';
DOM.subtitle.textContent = '';
/** 로딩 중... */
function showLoading() {DOM.loading.style.display = 'flex';}
function hideLoading() {DOM.loading.style.display = 'none';}

/** 모드 이름(string -> 타이틀) */
function modeName(s) {return s === 'advanced' ? '점프 투 데이터 팩(Adv)' : '점프 투 데이터 팩';}
/** 모드 숫자 -> 문자, 문자 -> 숫자 */
const modeMap = {menu: '0',basic: '1',advanced: '2'}
const modeMapReverse = Object.fromEntries(Object.entries(modeMap).map(([k, v]) => [v, k]));
function modeNumber(s) {return modeMap[s];}
function modeNumber2Name(s) {return modeMapReverse[s];}
...
function renderSidebar(list, depth = 0, searchQuery = '') {
   ...
   list.forEach(p => {
      ...
      a.innerHTML = `<strong>${titleHTML}</strong><div style="font-size:14px;color:var(--muted)">${summaryHTML || ''}</div>`;
      a.href = '#' + p.slug;
      ...
      if (p.children && Array.isArray(p.children)) {renderSidebar(p.children, depth + 1, searchQuery);}
   });
}
/** 사이드 바 페이지 하이라이트 */
function highlightActive() {
   const items = DOM.pages.querySelectorAll('.page-item');
   items.forEach(it => {it.classList.toggle('active', it.dataset.slug === AppState.currentSlug);});
}
...
async function loadPage(slug) {
   ...
   try {
      ...
      if (window.mermaid) {mermaid.init(undefined, DOM.article.querySelectorAll('.mermaid'));}

      attachLinkPreviews(); /** 링크 미리보기 로딩 */
      p.content = md;

      const query = DOM.search.value.trim();
      if (query) {DOM.article.innerHTML = highlight(DOM.article.innerHTML, query);}
   } catch (err) {
...
DOM.search.addEventListener('input', async e => {
   ...
   if (!q) {renderSidebar(AppState.pages); return;}
...
function highlight(text, q) {
   ...
   return text.replace(/(<[^>]+>)|([^<]+)/g, (match, tag, content) => {
      if (tag) {return tag;}
      else if (content) {return content.replace(regex, '<mark>$1</mark>');}
      return match;
   });
}
/** 슬래시 키로 검색 창 이동 */
...
function addCopyButtons() {
   ...
   codeBlocks.forEach(code => {
      ...
      // 위치 이동
      const updatePosition = () => {btn.style.left = (pre.scrollLeft + pre.clientWidth - btn.offsetWidth - 8) + 'px';};
      // 초기 위치
      updatePosition();
      // 스크롤 위치 이동
...
function replaceAnnotationsInArticle() {
   ...
   DOM.article.querySelectorAll("p").forEach(p => {
      ...
      p.replaceWith(Object.assign(document.createElement("div"), {
         ...
         innerHTML: `<span class="annotation-tag">@${match[1]}</span><span class="annotation-text">${marked.parseInline(match[2])}</span>`
      }));
   });
}
/** 이모지 (:sans:) */
